...
export default function SideScroller({ onNavigate, onLogout }: SideScrollerProps) {
  ...
  const slideAnim = useRef(new Animated.Value(-384)).current;
  const overlayAnim = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    Animated.parallel([
      Animated.timing(slideAnim, {
        toValue: isMenuOpen ? 0 : -384,
        duration: 320,
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
              {menuSections.map((section) => (
                <View key={section.id}>
                  {/* Section Header */}
                  <TouchableOpacity
                    className="flex-row items-center justify-between px-4 py-3 border-b border-neutral-200 dark:border-neutral-800"
                    onPress={() => handleMenuPress(section.id)}
                    activeOpacity={0.7}
                  >
                    <View className="flex-row items-center flex-1">
                      <Feather name={section.icon as any} size={22} color={iconColor} />
                      <Text className="text-lg font-semibold ml-3 text-neutral-900 dark:text-white">
                        {section.title}
                      </Text>
                    </View>
                    <Feather
                    ...
                    <View className="bg-gray-50 dark:bg-neutral-800">
                      {section.items.map((item) => (
                        <TouchableOpacity
                          ...
                          activeOpacity={0.7}
                        >
                          <Feather name={item.icon as any} size={18} color={isDark ? theme.colors['muted-dark'] : theme.colors.muted} />
                          <Text className="text-base ml-3" style={{ color: isDark ? theme.colors['muted-dark'] : theme.colors.muted }}>
                            {item.label}
                          </Text>
                        </TouchableOpacity>
                      ))}
